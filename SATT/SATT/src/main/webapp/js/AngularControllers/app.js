(function () {
    var aplicacionMundial = angular.module('aplicacionMundial', ['auth0', 'angular-storage', 'angular-jwt', 'ngRoute']);
...
aplicacionMundial.controller( 'LoginCtrl', function ( $scope, auth) {
  ...
  $location.path('/');
    ...
    aplicacionMundial.controller("getBoletines", function ($http, $scope) {
        $http.get('http://localhost:8081/service/boletines').
    ...
    aplicacionMundial.controller("getReportesSensores", function ($http, $scope) {
        $http.get('http://localhost:8081/service/reportes').
                success(function (data, status, headers, config) {
                    $scope.reportes = data;
    ...
    aplicacionMundial.controller("getEventosSismicos", function ($http, $scope) {
        $http.get('http://localhost:8081/service/eventosSismicos').
                success(function (data, status, headers, config) {
                    $scope.eventos = data;
                }).
                error(function (data, status, headers, config) {
